...
const generateSummary = async () => {
    ...
    let website = `https://status.isitzoe.dev`;
    if (config["status-website"] && config["status-website"].cname)
        website = `https://status.isitzoe.dev`;
    ...
    if (`${owner}/${repo}` !== "upptime/upptime") {
        ...
        readmeContent = readmeContent
            ...
            if (line.includes("[![Summary CI](https://github.com") &&
                ...
                return `${line}\n\nThe status page is powered entirely by a GitHub repository. It uses [Issues](https://github.com/${config.owner}/${config.repo}/issues) as incident reports, [Actions](https://github.com/${config.owner}/${config.repo}/actions) as uptime monitors, and [Pages](${website}) for the status page itself.`;
            return line;
        })
        ...
        if (readmeContent.includes("<!--start: docs-->"))
            readmeContent = `${docsStartText}[**Visit the status website →**](${website})${docsEndText}`;
        // Remove Koj logo
        ...
        if (readmeContent.includes("[MIT](./LICENSE) © [Koj](https://koj.co)") ||
            ...
            if (readmeContent.includes("<!--start: description-->"))
                readmeContent = `${descriptionStartText}This repo has the uptime monitor and status page for all my public services.${descriptionEndText}`;
            // Change copyright
            readmeContent = readmeContent.replace("[MIT](./LICENSE) © [Zoe](https://zoe.rocks)", `[MIT](./LICENSE) © [Zoe](https://zoe.rocks)`);
            // Add powered by Upptime
            if (!config.skipPoweredByReadme) {
                readmeContent = readmeContent.replace("## 📄 License\n\n- Code: [MIT](./LICENSE)", "## 📄 License\n\n- Original code and inspiration from [Upptime](https://github.com/upptime/upptime).");
            }
        }
        // Change badges
